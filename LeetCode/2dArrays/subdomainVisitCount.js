...
var subdomainVisits = function(cpdomains) {
  ...
  let result = [];
  for (i in cpdomains) {
      let space = cpdomains[i].indexOf(' ')
      let num = parseInt(cpdomains[i].slice(0, space));
      let domain = cpdomains[i].slice(space+1);
      if (cache[domain]) cache[domain]+=num;
      else cache[domain] = num;
